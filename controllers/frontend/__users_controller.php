...
//				if( !preg_match("/^[0-9]+$/", $key) ){
				...
				$word = $this->Word->alltrim( $val );		// 前後の空白を除去


				// add 2012.09.10 ryoshimura
				$key = str_replace ('word', '', $key);
//pr($key);
				$save = array();
				foreach( $words as $wVal ){
					if( $wVal['Word']['position'] == $key ){
						$save = $wVal;
						break;
					}
				}
				...
				if( empty($save) ){		// $saveが空の場合、新規レコード作成

					$save['Word']['search_word']	= $word;
					$save['Word']['position']		= $key;
					$save['Word']['user_uid']		= $user['User']['user_uid'];
					$this->Word->create();
					$this->Word->save( $save, false );

				} else if( $wVal['Word']['search_word'] !== $word ){	// 旧ワードを論理削除し、新規レコード作成
...
//				$save = array();
			...
			$this->data['User']['mail_address'] = $user['User']['mail_address'];
		}
