...
/**
* ScThrone専用スクレイピングモデル（といってもほとんどsc_public1。TOPと２ページ目のみ$sorceを整形）
*
* @param  none
* @return none
*/
class ScThrone extends Tracer {

	public $name = 'ScThrone';
...
//			$crlSite = ClassRegistry::init('Crlsite');
			...
			if( $site['IllegalSite']['search_url_post'] == null || $site['IllegalSite']['search_url_post'] === '' ){	// GET
				$html = &$crlSite->get_html( $url, $site, null, false, true );	// TOP及び２ページ目のHTMLが壊れてるのでタグ判定できないのでnull指定
				$sendMode = 'normal';

			} else {		//POST
				$html = &$crlSite->get_html( $url, $site, null, false, true, mb_convert_encoding($wd['Word']['search_word'], $site['IllegalSite']['encoding'], 'UTF-8' ) );	// TOP及び２ページ目のHTMLが壊れてるのでタグ判定できないのでnull指定
				$sendMode = 'post';
			...
			// throne専用 ってかHTML正しく書けや！ｗ
			$sorce	= strtolower( str_replace(array("\r\n","\r","\n"), '', $html->outertext) );
			$start	= strpos( $sorce, '<body>' );
			$end	= strpos( $sorce, '</body>', $start+1 ) + 7;
			$sorce	= substr( $sorce, $start, $end);
...
//		for( $p=1; $p<=1; $p++ ){	// クロールするページ（TOPから３ページ分）
			...
			$html = &$crlSite->get_html( $url, $site, null, false );	// スクレイピング
			if( !is_object($html) ){	// エラーが存在する場合
				if( isset($html['MonomiError']) ){	// エラーが存在する場合
...
//pr($obj);
			...
			foreach( $obj as $val ){		// 1投稿を括るタグを指定
				...
				// add 2012.09.23 notFoundが検出されない検索結果ページに備えて
				if( is_object( $val->find($site['IllegalSite']['sc_detail_link_tag']) )){
					if( empty( $val->find( $site['IllegalSite']['sc_detail_link_tag'], 0 )->find('a',0)->href ) ){
						$val->clear();
						unset($val);
						continue;
...
print_r( ' $detail_url: ' . $detail_url . "\n");
					...
					// throne専用 ってかHTML正しく書けや！ｗ
					$d_html = &$crlSite->get_html( $detail_url, $site, $site['IllegalSite']['sc_contents_tag'], false );	// スクレイピング
//					$d_html = &$crlSite->get_html( $detail_url, $site, null, false );	// スクレイピング
//pr($d_html->outertext);
/*					$sorce	= strtolower( str_replace(array("\r\n","\r","\n"), '', $d_html->outertext) );
					$start	= strpos( $sorce, '<body>' );
					$end	= strpos( $sorce, '</body>', $start+1 ) + 7;
					...
					$d_html->clear();
					unset($d_html);
					$d_html	= str_get_html( $sorce, "UTF-8", $site['IllegalSite']['encoding'] );
*/
...
//pr($content->plaintext);
					$crlSite->check_content_uni( $detail_url, $content, $site_uid, $site['IllegalSite']['site_url'] );
