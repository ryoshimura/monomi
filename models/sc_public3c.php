...
* ScPublic3c
*
* リストページでマッチングチェックを行ない、マッチした場合のみ詳細URLをスクレイピング
* キャッシュ＋<a>なしアップローダ含め全検出版
*
* @param  none
* @return none
*/
class ScPublic3c extends Tracer {

	public $name = 'ScPublic3c';
...
//print_r('loop End '.memory_get_usage() . "\n");
	...
	*/
	public function ispCrw( $site ) {
		...
		for( $p=1; $p<=2; $p++ ){	// クロールするページ（TOPから2ページ分）
...
//			$obj = $html->find('div.post');
			...
			foreach( $obj as $val ){		// 1投稿を括るタグを指定
				...
				if( is_object( $val->find( $site['IllegalSite']['sc_detail_link_tag'], $site['IllegalSite']['sc_detail_link_tag_order'] ) ) ){
					if( empty( $val->find( $site['IllegalSite']['sc_detail_link_tag'], $site['IllegalSite']['sc_detail_link_tag_order'] )->find('a',0)->href ) ){
						$val->clear();
						unset($val);
						continue;
					}
					$href = $val->find( $site['IllegalSite']['sc_detail_link_tag'], $site['IllegalSite']['sc_detail_link_tag_order'] )->find('a',0)->href;
...
print_r( ' $detail_url: ' . $detail_url . "\n");
					...
					$matching_text = '';
					try{
						if( is_object( $val->find( $site['IllegalSite']['sc_matching_text'], 0 ) ) ){
							if( empty( $val->find( $site['IllegalSite']['sc_matching_text'], 0 )->plaintext ) ){
								throw new Exception('ERROR empty()');
							} else {
								$matching_text = $val->find( $site['IllegalSite']['sc_matching_text'], 0 )->plaintext;
							}
						} else {
							throw new Exception('ERROR is_object()');
						}
					} catch (Exception $e) {
						$val->clear();
						unset($val);
					...
					// 判定対象文字列抽出し、ワードマッチ
					foreach( $aryWd as $wd ){
						if( false !== mb_strpos( strtolower($val->find( $site['IllegalSite']['sc_matching_text'], 0 )->plaintext), strtolower($wd['Word']['search_word'])) ){
							...
							$crlSite->check_content_noMatching( $detail_url, $content, $site_uid, $site['IllegalSite']['site_url'], $wd );
							$d_html->clear();
							unset($d_html);
						}
					}
