...
//				$html = &$crlSite->get_html( $url, $site, $site['IllegalSite']['sc_post_tag'], false, true );	// スクレイピング
				$html = &$crlSite->get_html( $url, $site, null, false, true );	// スクレイピング
				$sendMode = 'normal';
...
//				$html = &$crlSite->get_html( $url, $site, $site['IllegalSite']['sc_post_tag'], false, true, mb_convert_encoding($wd['Word']['search_word'], $site['IllegalSite']['encoding'], 'UTF-8' ) );	// スクレイピング
				$html = &$crlSite->get_html( $url, $site, null, false, true, mb_convert_encoding($wd['Word']['search_word'], $site['IllegalSite']['encoding'], 'UTF-8' ) );	// スクレイピング
				$sendMode = 'post';
			...
			// nyaa専用
			// nyaaは検索結果が１件の場合、詳細ページにリダイレクトする仕様のため
...
pr($obj_cnt);
//			if( !is_object($obj) ){			// エラー or 検索結果１件のみで詳細ページにリダイレクト
			if( $obj_cnt == 0 ){			// エラー or 検索結果１件のみで詳細ページにリダイレクト

//				$content = $html->find( $site['IllegalSite']['sc_contents_tag'], 0 );	// 詳細ページ用のタグで検索
				$content = $html->find( $site['IllegalSite']['sc_contents_tag'], $site['IllegalSite']['sc_contents_tag_order'] );
pr($html->outertext);
				if( !empty($content) ){	// 詳細ページなら
//					$content = $d_html->find( $site['IllegalSite']['sc_contents_tag'], 0 );
					$crlSite->check_content_uni( $detail_url, $content, $site_uid, $site['IllegalSite']['site_url'] );	// unicodeデコード
					$d_html->clear();
					unset($d_html);
					continue;
...
//					$detail_url = $href;
					$detail_url = $crlSite->conv_text_uni($href);		// unicodeデコード
