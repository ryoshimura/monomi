...
* ScPublic2c
*
* detailチェックは行なわない（リストページのみ）
*
* @param  none
* @return none
*/
class ScPublic2c extends Tracer {

	public $name = 'ScPublic2c';
	...
	*/
	public function ispCrw( $site ) {
		...
		for( $p=1; $p<=2; $p++ ){	// クロールするページ（TOPから３ページ分）
			...
			if( $p == 1 ){			// クロールするURLを選定
				if( $site['IllegalSite']['detail_url'] != null && $site['IllegalSite']['detail_url'] !== '' ){
					$url = $site['IllegalSite']['detail_url'];
				} else {
					$url = $site['IllegalSite']['site_url'];
				}
...
//			$obj = $html->find('div.post');
			...
			$flg = false;
			$cnt = 0;
			foreach( $obj as $d_html ){		// 1投稿を括るタグを指定
...
pr("IN!");
				...
				// add 2012.09.23 notFoundが検出されない検索結果ページに備えて
				if( is_object( $d_html->find( $site['IllegalSite']['sc_detail_link_tag'], $site['IllegalSite']['sc_detail_link_tag_order'] ) ) ){
					if( empty( $d_html->find( $site['IllegalSite']['sc_detail_link_tag'], $site['IllegalSite']['sc_detail_link_tag_order'] )->find('a',0)->href ) ){
						$d_html->clear();
						unset($d_html);
						continue;
					}
					$href = $d_html->find( $site['IllegalSite']['sc_detail_link_tag'], $site['IllegalSite']['sc_detail_link_tag_order'] )->find('a',0)->href;
				} else {
					$d_html->clear();
					unset($d_html);
					continue;
				...
				$crlSite->check_content( $detail_url, $d_html, $site_uid, $site['IllegalSite']['site_url'] );
